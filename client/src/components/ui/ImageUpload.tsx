...
import { UploadCloud, X, Loader2, FileImage, Check, Search, Link as LinkIcon } from 'lucide-react';
...
export const ImageUpload = ({ value, onChange, label = "Cover Image" }: ImageUploadProps) => {
  ...
  const openMediaModal = async (initialTab: 'library' | 'upload' = 'library') => {
  ...
  const handleSelectMediaConfirm = () => {
    const targetUrl = activeTab === 'url' ? customUrlInput : selectedMediaUrl;
    if (targetUrl) {
      onChange(targetUrl);
    }
    setIsMediaModalOpen(false);
  ...
  return (
    <div className="space-y-2">
      <div className="flex justify-between items-center">
        ...
        {!value && (
          <div className="flex gap-2">
            <button
              type="button"
              onClick={() => openMediaModal('library')}
        ...
        <div className="relative rounded-xl overflow-hidden border border-slate-200 aspect-video bg-slate-50 group">
          ...
          <div className="absolute top-2 right-2 flex gap-2">
            <button
              type="button"
              onClick={() => openMediaModal('library')}
              className="px-3 py-1.5 bg-white/95 backdrop-blur text-slate-800 hover:bg-brand-primary hover:text-white rounded-lg shadow-sm transition-colors text-xs font-bold flex items-center gap-1.5"
              title="Replace image from Media Library or Upload"
            >
      ...
      <Modal isOpen={isMediaModalOpen} onClose={() => setIsMediaModalOpen(false)} title="Choose Cover Image">
        <div className="space-y-4">
          ...
          <div className="flex border-b border-slate-200 gap-4">
            <button
              ...
              className={`pb-2 text-xs font-bold border-b-2 flex items-center gap-1.5 transition-colors ${
                activeTab === 'library' ? 'border-brand-primary text-brand-primary' : 'border-transparent text-slate-500 hover:text-slate-700'
              }`}
            >
              <FileImage className="w-3.5 h-3.5" /> Media Library
            </button>
            <button
              type="button"
              onClick={() => setActiveTab('upload')}
              className={`pb-2 text-xs font-bold border-b-2 flex items-center gap-1.5 transition-colors ${
                activeTab === 'upload' ? 'border-brand-primary text-brand-primary' : 'border-transparent text-slate-500 hover:text-slate-700'
              }`}
            >
              <UploadCloud className="w-3.5 h-3.5" /> Upload Direct
            </button>
            <button
              type="button"
              onClick={() => setActiveTab('url')}
              className={`pb-2 text-xs font-bold border-b-2 flex items-center gap-1.5 transition-colors ${
                activeTab === 'url' ? 'border-brand-primary text-brand-primary' : 'border-transparent text-slate-500 hover:text-slate-700'
              }`}
            >
                    ...
                    return (
                      ...
                      >
                        <img
                          ...
                          className="w-full h-full object-cover"
                        />
                        <div className="absolute inset-x-0 bottom-0 bg-slate-900/70 p-1 text-[10px] text-white truncate text-center font-mono">
                          {m.filename}
                        </div>
                        {isSelected && (
          ...
          {activeTab === 'url' && (
            <div className="space-y-2">
              ...
              <input
                ...
                className="w-full border border-slate-300 rounded-lg p-2.5 text-xs font-mono outline-none focus:ring-2 focus:ring-brand-primary/50"
              />
            </div>
          )}
          ...
          <div className="pt-4 flex justify-between items-center border-t border-slate-100">
            ...
            <div className="flex gap-2">
              ...
              <Button
                type="button"
                onClick={handleSelectMediaConfirm}
                disabled={activeTab === 'url' ? !customUrlInput : !selectedMediaUrl}
              >
                Use as Cover Image
              </Button>
            </div>
